...
const SAFE_PATTERNS = [
	...
	/^\s*htop\b/,
	/^\s*free\b/,
	/^\s*git\s+(status|log|diff|show|branch|remote\s+(-v|show|get-url)|config\s+--get)/i,
	/^\s*git\s+ls-/i,
	/^\s*npm\s+(list|ls|view|info|search|outdated|audit|test|run|exec)/i,
	/^\s*npx\b/i,
	/^\s*yarn\s+(list|info|why|audit|test|run)/i,
	/^\s*pnpm\s+(list|info|audit|test|run|exec)/i,
	/^\s*bun\s+(test|run)/i,
	/^\s*cargo\s+(test|check|build|clippy|fmt\s+--check)/i,
	/^\s*go\s+(test|vet|build|fmt)/i,
	/^\s*pytest\b/i,
